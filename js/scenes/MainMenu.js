...
export default class MainMenu extends Scene3D{
        constructor() {
          super({ key: "MainScene" });
        }
        ...
        walkAnimation() {
          if (this.robot.animation.current !== "Walking")
            this.robot.animation.play("Walking");
        }

        idleAnimation() {
          if (this.robot.animation.current !== "Idle")
            this.robot.animation.play("Idle");
        }
        ...
        update(time, delta) {
          ...
          if (this.robot && this.robot.body) {
            ...
            if (this.keys.a.isDown) {
              ...
              if (theta > -(Math.PI / 2))
                this.robot.body.setAngularVelocityY(-10);
              this.walkAnimation();
            }
            // move right
            ...
            if (
              this.keys.w.isDown &&
              this.robot.userData.onGround &&
              Math.abs(this.robot.body.velocity.y) < 1e-1
            ) {
